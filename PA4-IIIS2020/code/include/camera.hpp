...
public:
    ...
    // Auxiliary function.
    void rotateLevel(float dist, float angle) {
        Quat4f quat;
        quat.setAxisAngle(angle / 180.0 * 3.1415, up);
        Vector3f lookAtCenter = center + dist * direction;
        direction = Matrix3f::rotation(quat) * direction;
        horizontal = Matrix3f::rotation(quat) * horizontal;
        center = lookAtCenter - dist * direction;
    }

    virtual Vector2f project(const Vector3f& world) = 0;
...
public:
    ...
    Vector2f project(const Vector3f& world) override {
        Matrix3f R = getRotation();
        Vector3f camSpace = R.inverse() * (world - center);
        Vector2f pxSpace = camSpace.xy() / camSpace.z();
        pxSpace[0] = pxSpace[0] * fx + cx;
        pxSpace[1] = pxSpace[1] * fy + cy;
        return pxSpace;
    }
