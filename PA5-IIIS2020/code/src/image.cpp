...
int Image::LoadBMP(const char* filename)
{
    ...
    if (bmph.bfType[0] != 'B' && bmph.bfType[1] != 'M') return 0;
    ...
    reshape(bmph.biWidth, bmph.biHeight);
    int channels = 3;
    int offset = 0;
    int lineLength = bmph.biWidth * channels;
    offset = lineLength % 4;
    if (offset > 0) offset = 4 - offset;
    unsigned char pixVal;
	int ipos;
    for (int i = 0; i < bmph.biHeight; i++) {
        for (int j = 0; j < bmph.biWidth; j++) 
            for (int k = 0; k < 3; k++) {
                ipos = (bmph.biWidth * i + j);
                fread(&pixVal, 1, 1, file);
                data[ipos][2-k] = (float)pixVal / 255.0;
            }
