...
Object3D *SceneParser::parseObject(char token[MAX_PARSER_TOKEN_LENGTH]) {
    ...
    } else if (!strcmp(token, "FFDCage")) {
        answer = (Object3D *) parseFFDCage();
...
FFDCage *SceneParser::parseFFDCage() {
    char token[MAX_PARSER_TOKEN_LENGTH];
    getToken(token);
    assert (!strcmp(token, "{"));
    getToken(token);
    assert (!strcmp(token, "resolution"));
    int rx = readInt(), ry = readInt(), rz = readInt();
    getToken(token);
    assert (!strcmp(token, "geometry"));
    getToken(token);
    assert (!strcmp(token, "TriangleMesh"));
    Mesh* object = parseTriangleMesh();

    auto *answer = new FFDCage(object, rx, ry, rz);
    getToken(token);
    assert (!strcmp(token, "controls"));
    getToken(token);
        ...
        for (int tx = 0; tx <= rx; ++tx) {
            for (int ty = 0; ty <= ry; ++ty) {
                for (int tz = 0; tz <= rz; ++tz) {
                    ...
                    fToken = true;
                    answer->controls[tx][ty][tz] = readVector3f();
                    getToken(token);
                    assert (!strcmp(token, "]"));
                }
            }
