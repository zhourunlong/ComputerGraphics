...
Plane* editingPlane = nullptr;
FFDCage* editingCage = nullptr;
int edX, edY, edZ;
...
void keyboardFunc( unsigned char key, int x, int y ) {
    if (!isEditing && (key == 'z' || key == 'Z')) {
        editMode = !editMode;
    }
    glutPostRedisplay();
}

void setupFFDEditing(int x, int y) {
    // Determine which control point is used.
    Group* pGroup = sceneParser->getGroup();
    int nGroup = pGroup->getGroupSize();
    FFDCage* pCage = nullptr;
    int cx, cy, cz;
    float minDist = 1e6;
    for (int gi = 0; gi < nGroup; ++gi) {
        Object3D* object = pGroup->getObject(gi);
        if (object->isEditable()) {
            auto* ffdCage = dynamic_cast<FFDCage*>(object);
            for (int mx = 0; mx <= ffdCage->resX; ++mx) {
                for (int my = 0; my <= ffdCage->resY; ++my) {
                    for (int mz = 0; mz <= ffdCage->resZ; ++mz) {
                        float thisDist = (sceneParser->getCamera()->project(
                                ffdCage->controls[mx][my][mz]) - Vector2f(x, y)).abs();
                        if (thisDist < minDist) {
                            minDist = thisDist;
                            cx = mx; cy = my; cz = mz;
                            pCage = ffdCage;
    ...
    if (pCage) {
        pCage->update(cx, cy, cz, pCage->controls[cx][cy][cz]);
        delete editingPlane;
        Vector3f editPlaneNormal = sceneParser->getCamera()->getDirection();
        float editPlaneD = Vector3f::dot(editPlaneNormal, pCage->controls[cx][cy][cz]);
        editingPlane = new Plane(editPlaneNormal, editPlaneD, nullptr);
        editingCage = pCage;
        edX = cx; edY = cy; edZ = cz;
    }
}
...
void mouseFunc(int button, int state, int x, int y) {
    if (editMode) {
        ...
        // Non-edit mode. Use cameraController to handle input.
        if (state == GLUT_DOWN) {
            switch (button) {
...
void motionFunc(int x, int y) {
    if (editMode && editingCage) {
        Ray pRay = sceneParser->getCamera()->generateRay(Vector2f(x, imgH - y));
        Hit rayHit;
        editingPlane->intersect(pRay, rayHit, 0.0);
        Vector3f newPos = pRay.pointAtParameter(rayHit.getT());
        editingCage->update(edX, edY, edZ, newPos);
    } else {
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 3 && argc != 2) {
        cout << "Usage: PA4 <input scene file> [output image file]" << endl;
    ...
    glutCreateWindow("PA4 Free-form Deformation");
